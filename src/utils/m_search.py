...
def knowledge_base_retrieve(query: str, search_client: Any = None) -> list[dict[str, Any]]:
    ...
    """
    f_log(f"Executing local semantic hybrid search for: {query}", level="retrieve")
    ...
    if cap_dir.exists() and cap_dir.is_dir():
        for md_file in cap_dir.glob("*.md"):
            try:
                ...
                f_log(f"Failed to read capability file {md_file.name}: {e}", level="error")

    if not results:
        f_log("No specific capabilities matched. Returning default WAF guidance.", level="warning")
        results = [{"id": "default-waf", "content": "WAF Resiliency best practices suggest multi-region deployments."}]
